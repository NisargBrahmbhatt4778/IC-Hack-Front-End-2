...
const VideoPane: React.FC<VideoPaneProps> = ({ videoUrl }) => {
  ...
  return (
    <div className="h-screen bg-orange-100 p-6 flex items-center justify-center overflow-y-auto relative">
      ...
      <button
        onClick={() => router.push("/recommendations")} // Using router instead of page reload
        className="absolute bottom-6 left-6 bg-orange-600 text-white px-4 py-2 rounded-full shadow-lg hover:bg-orange-700 transition duration-300"
      >
        Go back
      </button>
    </div>
